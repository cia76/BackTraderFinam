    ...
    def get_bars_from_history(self) -> None:
        ...
        while True:  # Будем получать бары пока не получим все
            ...
            if 'candles' not in response_dict:  # Если бар нет в словаре
                ...
                return  # то выходим, дальше не продолжаем
            history_bars = response_dict['candles']  # Получаем все бары из Finam
            if len(history_bars) > 0:  # Если пришли новые бары
                first_bar_open_dt = self.get_bar_open_date_time(history_bars[0])  # Дату и время первого полученного бара переводим из UTC в МСК
                last_bar_open_dt = self.get_bar_open_date_time(history_bars[-1])  # Дату и время последнего полученного бара переводим из UTC в МСК
                self.logger.debug(f'Получены бары с {first_bar_open_dt} по {last_bar_open_dt}')
                for histoy_bar in history_bars:  # Пробегаемся по всем полученным барам
                    bar = dict(datetime=self.get_bar_open_date_time(histoy_bar),
                               ...
                               volume=int(histoy_bar['volume']))  # Бар из истории
                    if self.is_bar_valid(bar):  # Если исторический бар соответствует всем условиям выборки
                        self.history_bars.append(bar)  # то добавляем бар
